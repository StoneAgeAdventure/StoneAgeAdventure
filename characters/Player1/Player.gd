	...
	match get_state():
		...
		State.ATTACK:
			...
			if sprite.flip_h == true:
				$Weapon._set_weapon(3,true)
				$Weapon.position = Vector2(10,11)  #decide sword rotate center
				animation_player.play("attack_left")
			else :
				$Weapon._set_weapon(3,false)
				$Weapon.position = Vector2(17,13)  #decide sword rotate center
